...
if defined FASTAPI_VERBOSE (
  ...
  set "UVICORN_FLAGS=--reload --log-level info"
) else (
  set "PIP_FLAGS=--disable-pip-version-check --no-input -q"
  set "UVICORN_FLAGS=--reload --log-level warning --no-access-log"
)
...
.\.venv\Scripts\python.exe -m uvicorn main:app %UVICORN_FLAGS%
if %ERRORLEVEL% NEQ 0 (
