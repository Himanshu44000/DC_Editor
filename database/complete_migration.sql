...
CREATE TABLE collab_folders (
  ...
  created_by TEXT REFERENCES collab_users(id) ON DELETE CASCADE,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE collab_file_versions (
  ...
  changed_by TEXT REFERENCES collab_users(id) ON DELETE CASCADE,
...
CREATE TABLE collab_invites (
  ...
  invited_by TEXT REFERENCES collab_users(id) ON DELETE CASCADE,
  role_id TEXT NOT NULL DEFAULT 'viewer' REFERENCES collab_roles(id) ON DELETE RESTRICT,
  invited_email TEXT,
...
CREATE TABLE collab_audit_log (
  ...
  user_id TEXT REFERENCES collab_users(id) ON DELETE CASCADE,
...
-- ========================================
CREATE TABLE collab_refresh_tokens (
  id TEXT PRIMARY KEY,
  user_id TEXT NOT NULL REFERENCES collab_users(id) ON DELETE CASCADE,
...
CREATE INDEX idx_collab_refresh_tokens_user ON collab_refresh_tokens (user_id);
CREATE INDEX idx_collab_refresh_tokens_expires ON collab_refresh_tokens (expires_at);
